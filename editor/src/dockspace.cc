  ...
  void DockSpace::Draw(Window &window) {
    ...
    if (ImGui::BeginMenuBar()) {
      ...
      if (ImGui::BeginMenu("File")) {
        ...
        if (ImGui::MenuItem(ICON_FA_DOOR_OPEN " Quit", "C-q")) { data.terminal->quit_requested = true; }
        if (ImGui::IsItemHovered()) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
      ...
      if (ImGui::BeginMenu("Window")) {
        ...
        if (ImGui::MenuItem("Entities", nullptr, data.show_hierarchy)) { data.show_hierarchy = !data.show_hierarchy; }
        if (ImGui::IsItemHovered()) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);

        if (ImGui::MenuItem("Inspector", nullptr, data.show_inspector)) { data.show_inspector = !data.show_inspector; }
        if (ImGui::IsItemHovered()) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);

        if (ImGui::MenuItem("Renderer", nullptr, data.show_renderer)) { data.show_renderer = !data.show_renderer; }
        if (ImGui::IsItemHovered()) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);

        if (ImGui::MenuItem("Terminal", nullptr, data.show_terminal)) { data.show_terminal = !data.show_terminal; }
        if (ImGui::IsItemHovered()) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
    ...
    if (data.show_imgui_demo) { ImGui::ShowDemoWindow(&data.show_imgui_demo); }
  }
